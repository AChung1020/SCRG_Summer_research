...
base_name = os.path.splitext(args["image"])[0].replace("_thermal", "")

# Load the template image (thermal image)
template = cv2.imread('thermal/' + base_name + '_thermal.jpg')
template = preprocess_thermal_image(template)

# Perform edge detection on the entire template
edges_template = template

# Create a mask for the left half of the template
...
cv2.imshow("Masked Template", edges_template)
cv2.waitKey(0)
cv2.destroyAllWindows()
...
for scale in np.linspace(0.2, 1.0, 200)[::-1]:
    ...
    # cv2.imshow("Visible Image", edged)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
...
thermal_image = cv2.imread('thermal/' + base_name + '_thermal.jpg', cv2.IMREAD_COLOR)
...
output_filename = base_name + '_optical.jpg'
cv2.imwrite(os.path.join('./output/', output_filename), crop_img_resized)

# Concatenate and save the result
results_filename = base_name + '_optical.jpg'
final = np.concatenate((crop_img_resized, thermal_image), axis=1)
cv2.imwrite(os.path.join('./results/', results_filename), final)
