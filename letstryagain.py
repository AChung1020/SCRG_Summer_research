...
def alignImages(im1, im2, filename):
    ...
    # Crop images to the upper right quadrant
    im1_crop = im1[startY:endY, startX:endX]
    im2_crop = im2[startY:endY, startX:endX]

    # Convert cropped images to grayscale
    im1Gray = cv2.cvtColor(im1_crop, cv2.COLOR_BGR2GRAY)
    im2Gray = cv2.cvtColor(im2_crop, cv2.COLOR_BGR2GRAY)

    # Detect ORB features and compute descriptors in the cropped region
    ...
    # Draw top matches
    imMatches = cv2.drawMatches(im1_crop, keypoints1, im2_crop, keypoints2, matches, None)
    ...
    # Adjust points for the full image coordinate system
    points1[:, 0] += startX
    points2[:, 0] += startX
